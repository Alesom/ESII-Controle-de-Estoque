<?php
  ...
  if($_GET['saida'] == 1){
    $Saida = "SELECT p.nome AS nomep,p.cod as codp, sum(s.qtd) as soma
							FROM produto AS p
  ...
  if ($_GET['entrada'] == 1){
    $Entrada = "SELECT p.nome AS nomep,p.cod as codp, sum(e.qtd) as soma
							FROM produto AS p
              INNER JOIN insercao AS e on (p.cod = e.codp)
							INNER JOIN localizacao AS lz on (p.cod = lz.codp)
							INNER JOIN grupo AS g on (SUBSTRING(p.cod,1,1) = g.codg)
							INNER JOIN local AS l on (l.codl = lz.codl)
